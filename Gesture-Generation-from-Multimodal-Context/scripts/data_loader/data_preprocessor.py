        ...
        for key, value in cursor:
            ...
            for clip_idx, clip in enumerate(clips):
                ...
                for type in filtered_result.keys():
                    ...
                    print('===type: ',type)
                    print('===filtered_result[type]: ', filtered_result[type])
                    print('===n_filtered_out[type]: ', n_filtered_out[type])
        ...
        for i in range(num_subdivision):
            ...
            if len(sample_words) >= 2:
                ...
                if is_correct_motion or self.disable_filtering:
                    ...
                    #print(n_filtered_out[filtering_message], "--------filter message", filtering_message, "")


        #print(len(sample_skeletons_list), "========len list")
        if len(sample_skeletons_list) > 0:
            with self.dst_lmdb_env.begin(write=True) as txn:
                for words, poses, aux in zip(sample_words_list, sample_skeletons_list, aux_info):
                    ...
                    v = [words, poses, normalized_dir_vec, aux]
                    v = pyarrow.serialize(v).to_buffer()
                    txn.put(k, v)
                    self.n_out_samples += 1
                    print(self.n_out_samples, '========')
