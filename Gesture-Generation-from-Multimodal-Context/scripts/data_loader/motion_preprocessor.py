...
class MotionPreprocessor:
    ...
    def get(self):
        ...
        if self.skeletons != []:
            ...
            for i, frame in enumerate(self.skeletons):
                if np.isnan(self.skeletons[i]).any(): # missing joints
                    for j in range(len(self.skeletons[i])):
                        for k in range(len(self.skeletons[i][j])):
                            if np.isnan(self.skeletons[i][j][k]):
                                self.skeletons[i][j][k] = 0
    ...
    def check_spine_angle(self, verbose=False):
        ...
        if np.rad2deg(max(angles)) > 100 or np.rad2deg(np.mean(angles)) > 90:  # exclude 4495
