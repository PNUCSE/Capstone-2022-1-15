...
def main(mode, checkpoint_path, option):
    ...
    if mode == 'eval':
        ...
        examples = [
            '많이 배고파?',
            '부산대학교 학생 여러분, 안녕하십니까?, 2022학년도 전교 부회장 후보 기호 1번 황원식입니다. ... 여러분! 여러분은 언제 가장 행복하고 즐거우신가요? 저는 지금 이 순간이 너무 벅차고 행복합니다. 학우를 위해, 학교를 위해 그리고 저를 위해'
            '지금 이 자리에 섰습니다. 지금 이 순간은 학우와 학교가 바라는 것과 베가 바라는 모든 것을 이룰 수 있는 순간이기 때문입니다.',
