...
def train_iter_gan(args, epoch, in_text, target_poses, vid_indices,
    ...
    if epoch > warm_up_epochs and args.loss_gan_weight > 0.0:
        ...
        out_dir_vec, *_ = pose_decoder(pre_seq, in_text, vid_indices)  # out shape (batch x seq x dim)
    ...
    # decoding
    out_dir_vec, z, z_mu, z_logvar = pose_decoder(pre_seq, in_text, vid_indices)
    ...
    if (args.z_type == 'speaker' or args.z_type == 'random') and args.loss_reg_weight > 0.0:
        ...
        out_dir_vec_rand_vid, z_rand_vid, _, _ = pose_decoder(pre_seq, in_text, rand_vids)
        beta = 0.05
